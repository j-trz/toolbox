document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('btnItinerarioSPRK').addEventListener('click', function() {
      Alerta();
      generateTable();
  });
...
function generateTable() {
  ...
  let table = "<table style='margin-bottom: 15px; font-size: 12px;font-family: 'Montserrat', sans-serif;font-weight: 500;color: #2c4b8b;border-collapse: collapse;'><thead class='table-dark' style='text-align: center;font-size: 12px;'><tr><th style='padding:5px;'>Aerolínea</th><th style='padding:5px;'>Número de vuelo</th><th style='padding:5px;'>Aeropuerto de origen</th><th style='padding:5px;'>Aeropuerto de destino</th><th style='padding:5px;'>Fecha de vuelo</th><th style='padding:5px;'>Hora de salida</th><th style='padding:5px;'>Hora de llegada</th></tr></thead><tbody>";



  for (let i = 0; i < text.length; i++) {
    let flight = text[i].trim().split(/\s+/);
    let flightNum = flight[1]; 
    let origin = flight[3];
    let destination = flight[5];
    let flightDate = flight[6]+flight[7];
    let departureTime = flight[8];
    let arrivalTime = '';

    const dateRegex = /^\d{2}[A-Z]{3}.$/;
    if (dateRegex.test(flight[9])) {
      arrivalTime = flight[10]+ "<span class='day'>+1</span>";
    } else {
      arrivalTime = flight[9];
    }
    ...
    const airportCodes = {
      ...
      function replaceAirportCodes(text) {
